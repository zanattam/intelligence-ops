    ...
    internal class Program
    {
        ...
        static bool Login()
        {
            ...
            else
            {
                ...
                TypeEffect(" EQUIPE ENVIADA PARA NEUTRALIZAÇÃO");
                Thread.Sleep(2000);
                return false;
            }
        }
        ...
        static void ShowMissionStory(Mission mission)
        {
            ...
            Console.WriteLine($"Objetivo: {mission.Objective}\n");
                        ...
                        string[] intros = {
                    "Inteligência confirmou a presença de forças inimigas fortemente armadas.",
                    "Relatórios indicam patrulhas constantes na área alvo.",
                    "O inimigo reforçou sua base recentemente, cuidado redobrado!"
                };

                        string[] complications = {
                    "O operador pode ser detectado a qualquer momento.",
                    "Uma armadilha foi localizada próximo ao objetivo.",
                    "O tempo é curto, cada decisão conta."
                };

                        string[] outcomes = {
                    "Neutralize os guardas e complete a missão sem alertar inimigos.",
                    "Extraia os dados vitais e evite confronto direto.",
                    "Evacue o VIP antes que o inimigo perceba a infiltração."
                };

            var rng = new Random();
            Console.WriteLine("Narrativa:");
            Console.WriteLine($"{intros[rng.Next(intros.Length)]} {complications[rng.Next(complications.Length)]} {outcomes[rng.Next(outcomes.Length)]}\n");
        ...
        static void GenerateCampaign()
        {
            ...
            Database.AddCampaignMission("Operação Bravo", "Neutralizar base inimiga", Difficulty.Media,5);
            Database.AddCampaignMission("Operação Alpha", "Recuperar intel secreto", Difficulty.Facil,6);
            Database.AddCampaignMission("Operação Charlie", "Destruir suprimentos inimigos", Difficulty.Dificil,7);
            Database.AddCampaignMission("Operação Delta", "Evacuação de VIP", Difficulty.Media,8);
            Database.AddCampaignMission("Operação Echo", "Proteção de civis", Difficulty.Facil,9);
